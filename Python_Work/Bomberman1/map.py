class Map(object):
    ...
    def __init__(self, mapp, width, height, dblocks={}, rewards={}, forbidden_positions_dblocks=[]):
        self.__map = mapp[:]
        self._width = width
        self._height = height
    ...
    def __position_allowed_for_dblocks(self, pos):
        return (self.__map[pos] is None) and (pos not in self.__forbidden_positions_dblocks)
    ...
    def __fill_map_with_dblocks_and_rewards(self):
        ...
        for x, y in iter(self.__rewards):
            if y > 0:
                rewards += [x] * y
        shuffle(rewards)
        ...
        for x, y in iter(self.__dblocks):
            if y > 0:
                dblocks += [x] * y
        shuffle(dblocks)    
    ...
    def has_dblock(self, row, col):
        pos = self.__convert_position(row, col)
        element = self.__map[pos]
        return isinstance(element, tuple) and element[0] is not None
        
    def has_reward(self, row, col):
        pos = self.__convert_position(row, col)
        element = self.__map[pos]
        return isinstance(element, tuple) and element[1] is not None
...
class PygameMap(Map):
    ...
    def __init__(self, map, width, height, screen, dblock_image, ublock_image, reward_image, tiles_width=16,
                 tiles_height=16, ublocks=[], dblocks={}, rewards={}, forbidden_positions_dblocks=[]):
